...
async function routes(app) {
    ...
    const defaultHcaptchaSecretKey =
    ...
    app.post("/user/login", async (req, res) => {
        ...
        await db.execute(
            "INSERT INTO sessions (user_id, `key`, expires) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 30 DAY))",
            [user.id, token]
        );
        db.release();
    ...
    app.get("/user/register", (_, res) => {
        res.view("views/register.pug", {
            serverName: process.env.SERVER_NAME,
            hcaptchaSiteKey:
                process.env.HCAPTCHA_SITE_KEY || defaultHcaptchaSiteKey,
        });
    });
    app.post("/user/register", async (req, res) => {
        ...
        const hcaptchaSecretKey =
            process.env.HCAPTCHA_SECRET_KEY || defaultHcaptchaSecretKey;
        const hcaptchaResult = await verify(
            hcaptchaSecretKey,
    ...
    app.get("/user/validate", async (req, res) => {
        ...
        return res.view("views/validate.pug", {
            session: session.value,
            csrf: csrf.value,
            userData: JSON.stringify(apiUser),
        });
    });
    ...
    app.post("/api/user/:userKey/friend-code/update", async (req) => {
        ...
        if (!req.body.name || !/^[a-zA-Z0-9]+$/.test(req.body.name)) {
            return {
                status: "error",
                message:
                    "You need to enter a valid name. No special characters!",
            };
        }
        const user = rows[0];
        ...
        if (friendCode === user.friend_code) {
            return {
                status: "error",
                message:
                    "Someone is already using that name with the same code as you. Please try again.",
            };
        }
        try {
            await db.execute("UPDATE users SET friend_code = ? WHERE id = ?", [
                friendCode,
                user.id,
            ]);
        } catch (e) {
